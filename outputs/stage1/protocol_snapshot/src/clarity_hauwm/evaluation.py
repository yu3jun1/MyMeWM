...
def _tertile_ratio(disagreements: Sequence[float], errors: Sequence[float]) -> float | None:
    if len(errors) < 3:
        return None
    count = len(errors) // 3
    order = np.argsort(np.asarray(disagreements), kind="mergesort")
...
def evaluate_run(run_dir: str | Path, kind: str, max_horizon: int = 3, device: str = "auto") -> dict:
    ...
    if kind == "recursive":
        ...
        if model.ensemble_size < 2:
            raise ValueError("Disagreement evaluation requires an ensemble variant")
