...
def summarize_horizon_ablation(input_dir: str | Path,
    ...
    for path in sorted((root / "horizon_ablation").glob("k*")):
        ...
        window_counts = tuple(by_horizon[h]["n_predictions"] for h in (1, 2, 3))
        if reference_windows is not None and window_counts != reference_windows:
            raise ValueError(f"H1-H3 evaluation windows differ across ablation runs: {path}")
        reference_windows = window_counts
...
def format_tables(prediction: dict, comparisons: dict, uncertainty: dict,
    ...
    if prediction:
        lines.extend(["", "Secondary Cosine Distance (mean ± std)",
                      "| Variant | Cos@1 | Cos@2 | Cos@3 |",
                      "|---|---:|---:|---:|"])
        for variant in VARIANTS:
            if variant in prediction:
                row = prediction[variant]
                lines.append(f"| {variant} | " + " | ".join(_fmt_stats(row[f"cos@{h}"])
                             for h in (1, 2, 3)) + " |")
    ...
    if ablation:
        ...
        if "K5" in ablation:
            lines.extend(["", "Optional K5 Stress Test (descriptive)", "| MSE@4 | MSE@5 |",
                          "|---:|---:|",
                          f"| {_fmt(ablation['K5'].get('mse@4'))} | {_fmt(ablation['K5'].get('mse@5'))} |"])
    return "\n".join(lines)
